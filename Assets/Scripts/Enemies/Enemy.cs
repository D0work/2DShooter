...
public class Enemy : MonoBehaviour
{
    ...
    [Tooltip("The Boundary for movement.")]
    public BoxCollider2D boundaryBox;
    [Tooltip("Delay before the movement.")]
    ...
    /// </summary>
    public enum MovementModes { Random, NoMovement, FollowTarget, Scroll };
    ...
    /// <summary>
    /// Description:
    /// Standard Unity function called once when the run starts before start
    /// Inputs: 
    /// none
    /// Returns: 
    /// void (no return)
    ...
    /// <summary>
    /// Description:
    /// Standard Unity function called once this is destroyed
    /// Input:
    /// none
    /// Return:
    /// void (no return)
    ...
    private void OnDestroy()
    {
        if (Buffs != null && Buffs.Count > 0)
        {
            ...
            if (roll < 0.5f)
            {
                Debug.Log("no buff spawn.");
                return;
            }
            float chancePerBuff = 0.5f / Buffs.Count; 
            float cumulative = 0.5f;
            for (int i = 0; i < Buffs.Count; i++)
            {
                cumulative += chancePerBuff;
                if (roll < cumulative)
                {
                    Instantiate(Buffs[i], transform.position, Quaternion.identity);
                    Debug.Log($"Buff {Buffs[i].name} spawné !");
                    break;
                }
            }
        }
    ...
    private Quaternion GetFollowPlayerRotation()
    {
        ...
        float angle = Vector3.SignedAngle(Vector3.down, (followTarget.position - transform.position).normalized, Vector3.forward) + 90f;
    ...
    /// <summary>
    /// Description:
    /// The direction and magnitude a random movement inside the boundary
    /// Inputs: 
    /// none
    /// Returns: 
    /// Vector3
    /// </summary>
    /// <returns>Vector3: The movement to be used in random movement mode.</returns>
    ...
    /// <summary>
    /// Description
    /// The random rotation of random movement mode
    /// Inputs: 
    /// none
    /// Returns: 
    /// Quaternion
